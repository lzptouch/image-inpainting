...
default=(r"E:\5.project\图像修复\经典\RFR-Inpainting-master\checkpoints\celeba\result3",r'E:\dataset\inpainting\celeba\test_choosed') ,nargs=2,
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    ...
    imgs = []
    for fn in files:
        fn = np.array(imageio.v2.imread(str(fn)).astype(np.float32))
        if len(fn.shape) < 3 :
            fn = gray2rgb(fn)
        imgs.append(fn)

    # imgs = np.array( [imageio.v2.imread(str(fn)).astype(np.float32) for fn in files])
    # imgs = [ gray2rgb(img)   if len(img.shape) < 3 else img for img in imgs   ]
    # Bring images to shape (B, 3, H, W)
    imgs = [i.transpose((2, 0, 1)) for i in imgs]
