...
def create_mask(width, height, mask_width, mask_height, x=None, y=None):
    # mask_width, mask_height决定了掩膜的宽高
    # x, y决定了掩膜的起始位置，如果没有则是随机指定
...
def imshow(img, title=''):
    # matplotlib.pyplot.gcf() 主要用于获取当前图形。
    # 如果没有可用的当前图形，那么将借助figure()函数创建一个
    fig = plt.gcf()
...
def stitch_images(inputs, *outputs, img_per_row=2):
    # inputs 和outputs都是tensor类型 input不止一张图片
    # 图片的间隔
    gap = 5
